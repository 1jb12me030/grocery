    ...
    def get(self,request):
            try:
                ...
                if username is not None:
                    obj=Category.objects.filter(user__username=username)
                    if obj is not None:
...
#                 username=request.GET.get('username')
#                 if username is not None:
#                     obj=Grocery.objects.filter(user__username=username)
#                     if obj is not None:
#                         serializer=GrocerySerializer(obj,many=True)
#                         return Response(data=serializer.data,status=status.HTTP_200_OK)
#                     else:
#                         return Response({'message':'Username not found'},status=status.HTTP_404_NOT_FOUND)
#                 else:
...
class ProductView(APIView):
    def get(self,request):
            try:
                username=request.GET.get('username')
                if username is not None:
                    obj=Product.objects.filter(user__username=username)
                    if obj is not None:
...
class CartView(APIView):
    def get(self,request):
            try:
                username=request.GET.get('username')
                if username is not None:
                    obj=Cart.objects.filter(user__username=username)
                    if obj is not None:
